...
// ClassLoader.h
//
// $Id: //percipio/1.4/Foundation/include/Percipio/ClassLoader.h#1 $
//
// Library: Foundation
...
#include "Percipio/MetaObject.h"
#include "Percipio/Manifest.h"
#include "Percipio/SharedLibrary.h"
...
class ClassLoader
	...
	///
	/// Starting with PERCIPIO release 1.3, a class library can
...
public:
	...
	void loadLibrary(const std::string& path, const std::string& manifest)
		...
		/// If the library exports a function named "percipioInitializeLibrary",
		...
		/// for the library to become unloaded.
	{
		...
		if (it == _map.end())
		{
			...
			try
			{
				std::string percipioBuildManifestSymbol("percipioBuildManifest");
				percipioBuildManifestSymbol.append(manifest);
				if (li.pLibrary->hasSymbol("percipioInitializeLibrary"))
				{
					InitializeLibraryFunc initializeLibrary = (InitializeLibraryFunc) li.pLibrary->getSymbol("percipioInitializeLibrary");
					initializeLibrary();
				}
				if (li.pLibrary->hasSymbol(percipioBuildManifestSymbol))
				{
					BuildManifestFunc buildManifest = (BuildManifestFunc) li.pLibrary->getSymbol(percipioBuildManifestSymbol);
	...
	void unloadLibrary(const std::string& path)
		...
		/// If the library exports a function named "percipioUninitializeLibrary",
		...
		/// for the library to become unloaded.
	{
		...
		if (it != _map.end())
		{
			...
			{
				if (it->second.pLibrary->hasSymbol("percipioUninitializeLibrary"))
				{
					UninitializeLibraryFunc uninitializeLibrary = (UninitializeLibraryFunc) it->second.pLibrary->getSymbol("percipioUninitializeLibrary");
					uninitializeLibrary();
				}
				delete it->second.pManifest;
