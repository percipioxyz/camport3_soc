...
// NumericString.h
//
// $Id: //percipio/1.4/Foundation/include/Percipio/NumericString.h#1 $
//
// Library: Foundation
// Package: Core
...
#include "Percipio/FPEnvironment.h"
#ifdef min
	#undef min
#endif
#ifdef max
...
#include <limits>
#include <cmath>
#if !defined(PERCIPIO_NO_LOCALE)
	#include <locale>
#endif


// binary numbers are supported, thus 64 (bits) + 1 (string terminating zero)
#define PERCIPIO_MAX_INT_STRING_LEN 65
// value from strtod.cc (double_conversion::kMaxSignificantDecimalDigits)
#define PERCIPIO_MAX_FLT_STRING_LEN 780

#define PERCIPIO_FLT_INF "inf"
#define PERCIPIO_FLT_NAN "nan"
#define PERCIPIO_FLT_EXP 'e'
	...
	/// Formatted string has at least [width] total length.
{
	...
	size = ptr - result;
	percipio_assert_dbg (size <= ptr.span());
	percipio_assert_dbg ((-1 == width) || (size >= size_t(width)));
	*ptr-- = '\0';
...
{
	char res[PERCIPIO_MAX_INT_STRING_LEN] = {0};
	std::size_t size = PERCIPIO_MAX_INT_STRING_LEN;
