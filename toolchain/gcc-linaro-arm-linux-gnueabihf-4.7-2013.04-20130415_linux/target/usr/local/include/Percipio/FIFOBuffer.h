...
// FIFOBuffer.h
//
// $Id: //percipio/1.4/Foundation/include/Percipio/FIFOBuffer.h#2 $
//
// Library: Foundation
// Package: Core
// Module:  FIFOBuffer
...
public:
	...
	mutable Percipio::BasicEvent<bool> writable;
	...
	mutable Percipio::BasicEvent<bool> readable;
	...
	std::size_t peek(Percipio::Buffer<T>& buffer, std::size_t length = 0) const
		/// Peeks into the data currently in the FIFO
		/// without actually extracting it.
	...
	std::size_t read(Percipio::Buffer<T>& buffer, std::size_t length = 0)
		/// Copies the data currently in the FIFO
		...
		/// Returns the length of data written.
	{
		...
		_used += len;
		percipio_assert (_used <= _buffer.size());
		if (_notify) notify(usedBefore);
	...
	{
		percipio_check_ptr(ptr);
		if (0 == length) return;

		Mutex::ScopedLock lock(_mutex);
		
		if (length > available())
			throw Percipio::InvalidAccessException("Cannot extend buffer.");
		
		if (!isWritable())
			throw Percipio::InvalidAccessException("Buffer not writable.");
