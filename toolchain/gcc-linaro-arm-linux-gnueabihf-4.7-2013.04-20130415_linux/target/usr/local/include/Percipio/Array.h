...
// Array.h
//
// $Id: //percipio/svn/Foundation/include/Percipio/Array.h#2 $
//
// Library: Foundation
// Package: Core
// Module:  Array
...
public:
	...
	{ 
		percipio_assert( i < N && "out of range" ); 
		return elems[i];
	}
	...
	reference at(size_type i)
		/// Element access with range check. Throws Percipio::InvalidArgumentException if the index is over range.
	{ 
		if(i>=size())
			throw Percipio::InvalidArgumentException("Array::at() range check failed: index is over range");
		return elems[i]; 
	}
