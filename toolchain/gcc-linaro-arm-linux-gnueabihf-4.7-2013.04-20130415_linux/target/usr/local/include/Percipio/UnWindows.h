...
// UnWindows.h
//
// $Id: //percipio/1.4/Foundation/include/Percipio/UnWindows.h#1 $
//
// Library: Foundation
// Package: Core
// Module:  UnWindows
...
// or not. PERCIPIO always calls the Unicode or ASCII functions directly (depending
// on whether PERCIPIO_WIN32_UTF8 is #define'd or not), so the macros are not ignored.
//
...
// GetUserName. Depending on whether "Percipio/UnWindows.h" has been included by a particular
// translation unit or not, this might be changed to GetUserNameA/GetUserNameW, or not.
// While, due to naming conventions used, this is less of a problem in PERCIPIO, some
// of the users of PERCIPIO might use a different naming convention where this can become
// a problem.
//
// To disable the #undef's, compile PERCIPIO with the PERCIPIO_NO_UNWINDOWS macro #define'd.
//
...
// Reduce bloat
#if defined(_WIN32)
	#if !defined(WIN32_LEAN_AND_MEAN) && !defined(PERCIPIO_BLOATED_WIN32)
...
// definitions.) For more information, see SdkDdkVer.h.
	...
	// If allowed (see PERCIPIO_MIN_WINDOWS_OS_SUPPORT
...
// #define PERCIPIO_FORCE_MIN_WINDOWS_OS_SUPPORT


#include <windows.h>


#if !defined(PERCIPIO_NO_UNWINDOWS)
