...
// PBKDF2Engine.h
//
// $Id: //percipio/1.4/Foundation/include/Percipio/PBKDF2Engine.h#1 $
//
// Library: Foundation
// Package: Crypt
// Module:  PBKDF2Engine
...
#include "Percipio/DigestEngine.h"
#include "Percipio/ByteOrder.h"
#include <algorithm>
...
public:
	...
	PBKDF2Engine(const std::string& salt, unsigned c = 4096, Percipio::UInt32 dkLen = PRF_DIGEST_SIZE):
		_s(salt),
		_c(c),
		_dkLen(dkLen)
	{		
...
protected:
	...
	void f(Percipio::UInt32 i)
	{
		PRF prf(_p);
		prf.update(_s);
		Percipio::UInt32 iBE = Percipio::ByteOrder::toBigEndian(i);
		prf.update(&iBE, sizeof(iBE));
		Percipio::DigestEngine::Digest up = prf.digest();
		Percipio::DigestEngine::Digest ux = up;
		percipio_assert_dbg(ux.size() == PRF_DIGEST_SIZE);
		for (unsigned k = 1; k < _c; k++)
		{
			prf.reset();
			prf.update(&up[0], up.size());
			Percipio::DigestEngine::Digest u = prf.digest();
			percipio_assert_dbg(u.size() == PRF_DIGEST_SIZE);
