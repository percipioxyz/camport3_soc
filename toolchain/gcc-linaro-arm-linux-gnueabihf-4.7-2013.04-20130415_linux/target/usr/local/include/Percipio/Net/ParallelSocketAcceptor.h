...
//
// $Id: //percipio/1.4/Net/include/Percipio/Net/ParallelSocketAcceptor.h#1 $
//
// Library: Net
// Package: Reactor
...
#include "Percipio/Net/ParallelSocketReactor.h"
#include "Percipio/Net/StreamSocket.h"
#include "Percipio/Net/ServerSocket.h"
#include "Percipio/Environment.h"
...
class ParallelSocketAcceptor
	...
	/// description see Percipio::Net::SocketAcceptor documentation.
	/// 
...
{
public:
	typedef Percipio::Net::ParallelSocketReactor<SR> ParallelReactor;

	explicit ParallelSocketAcceptor(ServerSocket& socket,
		unsigned threads = Percipio::Environment::processorCount()):
		_socket(socket),
		_pReactor(0),
		_threads(threads),
		_next(0)
		...
		/// with the given SocketReactor.
	{
		...
		_pReactor->addEventHandler(_socket,
			Percipio::Observer<ParallelSocketAcceptor,
