...
// HTTPCredentials.h
//
// $Id: //percipio/1.4/Net/include/Percipio/Net/HTTPCredentials.h#4 $
//
// Library: Net
// Package: HTTP
...
#include "Percipio/Net/HTTPDigestCredentials.h"
...
class Net_API HTTPCredentials
	...
	///     Percipio::Net::HTTPCredentials creds("user", "s3cr3t");
	/// 
	/// Second, send the HTTP request with Percipio::Net::HTTPClientSession.
	///     Percipio::Net::HTTPClientSession session("percipioproject.org");
	///     Percipio::Net::HTTPRequest request(HTTPRequest::HTTP_GET, "/index.html", HTTPMessage::HTTP_1_1);
	///     session.sendRequest(request);
	///     Percipio::Net::HTTPResponse;
	///     std::istream& istr = session.receiveResponse(response);
	///
	/// If the server responds with a 401 status, authenticate the 
	/// request and resend it:
	///     if (response.getStatus() == Percipio::Net::HTTPResponse::HTTP_UNAUTHORIZED)
	///     {
...
public:
	...
	static void extractCredentials(const Percipio::URI& uri, std::string& username, std::string& password);
