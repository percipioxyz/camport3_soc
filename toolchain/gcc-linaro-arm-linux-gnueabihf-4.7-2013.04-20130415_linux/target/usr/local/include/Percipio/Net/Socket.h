...
// Socket.h
//
// $Id: //percipio/1.4/Net/include/Percipio/Net/Socket.h#2 $
//
// Library: Net
// Package: Sockets
// Module:  Socket
...
public:
	...
	static int select(SocketList& readList, SocketList& writeList, SocketList& exceptList, const Percipio::Timespan& timeout);
	...
	bool poll(const Percipio::Timespan& timeout, int mode) const;
	...
	void setSendTimeout(const Percipio::Timespan& timeout);
		/// Sets the send timeout for the socket.
	
	Percipio::Timespan getSendTimeout() const;
	...
	void setReceiveTimeout(const Percipio::Timespan& timeout);
		/// Sets the send timeout for the socket.
		///
		/// On systems that do not support SO_RCVTIMEO, a
		/// workaround using poll() is provided.
	
	Percipio::Timespan getReceiveTimeout() const;
	...
	void getOption(int level, int option, Percipio::Timespan& value) const;
		/// Returns the value of the socket option 
		/// specified by level and option.
...
#if defined(PERCIPIO_HAVE_FD_POLL)
class FDCompare
...
inline bool Socket::poll(const Percipio::Timespan& timeout, int mode) const
{
...
inline void Socket::setSendTimeout(const Percipio::Timespan& timeout)
{
	_pImpl->setSendTimeout(timeout);
}


inline Percipio::Timespan Socket::getSendTimeout() const
{
	return _pImpl->getSendTimeout();
}


inline void Socket::setReceiveTimeout(const Percipio::Timespan& timeout)
{
	_pImpl->setReceiveTimeout(timeout);
}


inline Percipio::Timespan Socket::getReceiveTimeout() const
{
...
inline void Socket::setOption(int level, int option, const Percipio::Timespan& value)
{
	_pImpl->setOption(level, option, value);
}
...
inline void Socket::getOption(int level, int option, Percipio::Timespan& value) const
{
	_pImpl->getOption(level, option, value);
}
...
inline percipio_socket_t Socket::sockfd() const
{
