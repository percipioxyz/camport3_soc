...
// NetworkInterface.h
//
// $Id: //percipio/1.4/Net/include/Percipio/Net/NetworkInterface.h#4 $
//
// Library: Net
// Package: Sockets
...
#ifdef PERCIPIO_NET_HAS_INTERFACE
...
public:
	...
	typedef Percipio::Tuple<IPAddress, IPAddress, IPAddress> AddressTuple;
  ...
  std::string macAddressStr() const;
		/// Returns MAC (Media Access Control) address for the interface.
  ...
  static std::string macAddress2string(const MACAddress& mac);
...
private:
	...
	static Percipio::FastMutex _mutex;
};
...
inline std::string NetworkInterface::macAddressStr() const
{
  return macAddress2string(macAddress());
}
...
Net_API std::ostream& operator<<(std::ostream& os, const Percipio::Net::NetworkInterface::MACAddress& mac);


#endif // PERCIPIO_NET_HAS_INTERFACE
