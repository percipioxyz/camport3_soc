...
// SocketAcceptor.h
//
// $Id: //percipio/1.4/Net/include/Percipio/Net/SocketAcceptor.h#1 $
//
// Library: Net
// Package: Reactor
...
public:
	...
	{
		_pReactor->addEventHandler(_socket, Percipio::Observer<SocketAcceptor,
		...
		/// Destroys the SocketAcceptor.
	{
		try
		{
			if (_pReactor)
			{
				_pReactor->removeEventHandler(_socket, Percipio::Observer<SocketAcceptor,
					ReadableNotification>(*this, &SocketAcceptor::onAccept));
			}
		}
		catch (...)
	...
	{
		_pReactor = &reactor;
		if (!_pReactor->hasEventHandler(_socket, Percipio::Observer<SocketAcceptor,
			ReadableNotification>(*this, &SocketAcceptor::onAccept)))
		{
	...
	{
		if (_pReactor)
			throw Percipio::InvalidAccessException("Acceptor already registered.");

		_pReactor = &reactor;
		_pReactor->addEventHandler(_socket, Percipio::Observer<SocketAcceptor, ReadableNotification>(*this, &SocketAcceptor::onAccept));
	}
		...
		/// implementation or directly unregister the accept handler.
	{
		if (_pReactor)
		{
			_pReactor->removeEventHandler(_socket, Percipio::Observer<SocketAcceptor, ReadableNotification>(*this, &SocketAcceptor::onAccept));
		}
	}
