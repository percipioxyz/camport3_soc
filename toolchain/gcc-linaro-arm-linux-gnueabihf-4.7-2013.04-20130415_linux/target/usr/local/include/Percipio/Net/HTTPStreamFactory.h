...
// HTTPStreamFactory.h
//
// $Id: //percipio/1.4/Net/include/Percipio/Net/HTTPStreamFactory.h#1 $
//
// Library: Net
// Package: HTTP
...
#include "Percipio/URIStreamFactory.h"
...
class Net_API HTTPStreamFactory: public Percipio::URIStreamFactory
...
public:
	...
	HTTPStreamFactory(const std::string& proxyHost, Percipio::UInt16 proxyPort = HTTPSession::HTTP_PORT);
		/// Creates the HTTPStreamFactory.
		///
		/// HTTP connections will use the given proxy.

	HTTPStreamFactory(const std::string& proxyHost, Percipio::UInt16 proxyPort, const std::string& proxyUsername, const std::string& proxyPassword);
		/// Creates the HTTPStreamFactory.
		///
	...
	virtual std::istream* open(const Percipio::URI& uri);
