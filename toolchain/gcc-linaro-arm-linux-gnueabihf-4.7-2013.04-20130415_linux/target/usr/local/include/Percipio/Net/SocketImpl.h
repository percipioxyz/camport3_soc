...
// SocketImpl.h
//
// $Id: //percipio/1.4/Net/include/Percipio/Net/SocketImpl.h#4 $
//
// Library: Net
// Package: Sockets
// Module:  SocketImpl
...
class Net_API SocketImpl: public Percipio::RefCountedObject
...
public:
	...
	virtual void connect(const SocketAddress& address, const Percipio::Timespan& timeout);
	...
	virtual bool poll(const Percipio::Timespan& timeout, int mode);
	...
	virtual void setSendTimeout(const Percipio::Timespan& timeout);
		/// Sets the send timeout for the socket.
	
	virtual Percipio::Timespan getSendTimeout();
	...
	virtual void setReceiveTimeout(const Percipio::Timespan& timeout);
		/// Sets the send timeout for the socket.
		///
		/// On systems that do not support SO_RCVTIMEO, a
		/// workaround using poll() is provided.
	
	virtual Percipio::Timespan getReceiveTimeout();
	...
	void setOption(int level, int option, const Percipio::Timespan& value);
		/// Sets the socket option specified by level and option
	...
	virtual void setRawOption(int level, int option, const void* value, percipio_socklen_t length);
		/// Sets the socket option specified by level and option
		/// to the given time value.
	...
	void getOption(int level, int option, Percipio::Timespan& value);
		/// Returns the value of the socket option 
		/// specified by level and option.
	...
	virtual void getRawOption(int level, int option, void* value, percipio_socklen_t& length);
		/// Returns the value of the socket option 
		/// specified by level and option.	
	...
	percipio_socket_t sockfd() const;
		/// Returns the socket descriptor for the
		/// underlying native socket.

	void ioctl(percipio_ioctl_request_t request, int& arg);
		/// A wrapper for the ioctl system call.

	void ioctl(percipio_ioctl_request_t request, void* arg);
		/// A wrapper for the ioctl system call.

#if defined(PERCIPIO_OS_FAMILY_UNIX)
	int fcntl(percipio_fcntl_request_t request);
		/// A wrapper for the fcntl system call.

	int fcntl(percipio_fcntl_request_t request, long arg);
		/// A wrapper for the fcntl system call.
#endif
...
protected:
	...
	SocketImpl(percipio_socket_t sockfd);
	...
	void reset(percipio_socket_t fd = PERCIPIO_INVALID_SOCKET);
...
private:
	...
	percipio_socket_t _sockfd;
#if defined(PERCIPIO_BROKEN_TIMEOUTS)
	Percipio::Timespan _recvTimeout;
	Percipio::Timespan _sndTimeout;
#endif
	bool          _blocking;
...
// inlines
//
inline percipio_socket_t SocketImpl::sockfd() const
{
	return _sockfd;
}
...
{
	return _sockfd != PERCIPIO_INVALID_SOCKET;
}
