...
// SocketReactor.h
//
// $Id: //percipio/1.4/Net/include/Percipio/Net/SocketReactor.h#1 $
//
// Library: Net
// Package: Reactor
...
class Net_API SocketReactor: public Percipio::Runnable
...
public:
	...
	explicit SocketReactor(const Percipio::Timespan& timeout);
	...
	void setTimeout(const Percipio::Timespan& timeout);
		/// Sets the timeout. 
		///
	...
	void addEventHandler(const Socket& socket, const Percipio::AbstractObserver& observer);
		/// Registers an event handler with the SocketReactor.
		///
		/// Usage:
		///     Percipio::Observer<MyEventHandler, SocketNotification> obs(*this, &MyEventHandler::handleMyEvent);
		///     reactor.addEventHandler(obs);

	bool hasEventHandler(const Socket& socket, const Percipio::AbstractObserver& observer);
		/// Returns true if the observer is reistered with SocketReactor for the given socket.

	void removeEventHandler(const Socket& socket, const Percipio::AbstractObserver& observer);
...
private:
	typedef Percipio::AutoPtr<SocketNotifier>     NotifierPtr;
	typedef Percipio::AutoPtr<SocketNotification> NotificationPtr;
