...
// HTTPClientSession.h
//
// $Id: //percipio/1.4/Net/include/Percipio/Net/HTTPClientSession.h#8 $
//
// Library: Net
// Package: HTTPClient
...
public:
	...
	HTTPClientSession(const std::string& host, Percipio::UInt16 port = HTTPSession::HTTP_PORT);
		/// Creates a HTTPClientSession using the given host and port.

	HTTPClientSession(const std::string& host, Percipio::UInt16 port, const ProxyConfig& proxyConfig);
	...
	void setPort(Percipio::UInt16 port);
		/// Sets the port number of the target HTTP server.
		///
		/// The port number must not be changed once there is an
		/// open connection to the server.
	
	Percipio::UInt16 getPort() const;
		/// Returns the port number of the target HTTP server.

	void setProxy(const std::string& host, Percipio::UInt16 port = HTTPSession::HTTP_PORT);
		/// Sets the proxy host name and port number.
		
	void setProxyHost(const std::string& host);
		/// Sets the host name of the proxy server.
		
	void setProxyPort(Percipio::UInt16 port);
		/// Sets the port number of the proxy server.
		
	const std::string& getProxyHost() const;
		/// Returns the proxy host name.
		
	Percipio::UInt16 getProxyPort() const;
	...
	void setKeepAliveTimeout(const Percipio::Timespan& timeout);
		/// Sets the connection timeout for HTTP connections.
		
	const Percipio::Timespan& getKeepAliveTimeout() const;
	...
	virtual std::istream& receiveResponse(HTTPResponse& response);
		...
		/// in a Percipio::Net::MessageException being thrown.
		///
		/// In case a network or server failure happens
...
private:
	...
	Percipio::Timestamp _lastRequest;
	bool            _reconnect;
	bool            _mustReconnect;
	bool            _expectResponseBody;
	Percipio::SharedPtr<std::ostream> _pRequestStream;
	Percipio::SharedPtr<std::istream> _pResponseStream;
...
inline Percipio::UInt16 HTTPClientSession::getPort() const
{
	return _port;
}
...
inline Percipio::UInt16 HTTPClientSession::getProxyPort() const
{
...
inline const Percipio::Timespan& HTTPClientSession::getKeepAliveTimeout() const
{
