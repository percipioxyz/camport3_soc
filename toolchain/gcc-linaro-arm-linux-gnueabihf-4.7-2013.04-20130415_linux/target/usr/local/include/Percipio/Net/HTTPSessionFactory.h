...
//
// $Id: //percipio/1.4/Net/include/Percipio/Net/HTTPSessionFactory.h#1 $
//
// Library: Net
// Package: HTTPClient
...
#include "Percipio/SingletonHolder.h"
...
public:
	...
	HTTPSessionFactory(const std::string& proxyHost, Percipio::UInt16 proxyPort);
	...
	HTTPClientSession* createClientSession(const Percipio::URI& uri);
	...
	Percipio::UInt16 proxyPort() const;
		/// Returns the proxy port number, if one has been set, or zero otherwise.

	void setProxy(const std::string& proxyHost, Percipio::UInt16 proxyPort);
...
inline Percipio::UInt16 HTTPSessionFactory::proxyPort() const
{
	return _proxyPort;
}
