...
//
// $Id: //percipio/1.4/Foundation/include/Percipio/NestedDiagnosticContext.h#1 $
//
// Library: Foundation
// Package: Core
...
class Foundation_API NestedDiagnosticContext
	...
	/// at the end.
	/// The percipio_ndc(info) macro augments the information with a
...
// helper macros
//
#define percipio_ndc(func) \
	Percipio::NDCScope _theNdcScope(#func, __LINE__, __FILE__)


#if defined(_DEBUG)
	#define percipio_ndc_dbg(func) \
		Percipio::NDCScope _theNdcScope(#func, __LINE__, __FILE__)
#else
	#define percipio_ndc_dbg(func)
#endif
