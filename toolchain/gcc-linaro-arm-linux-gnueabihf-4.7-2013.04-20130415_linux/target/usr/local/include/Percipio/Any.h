...
// Any.h
//
// $Id: //percipio/1.4/Foundation/include/Percipio/Any.h#1 $
//
// Library: Foundation
// Package: Core
// Module:	Any
...
// Extracted from Boost 1.33.1 lib and adapted for percipio: Peter Schojer/AppliedInformatics 2006-02-02
//
...
#include "Percipio/MetaProgramming.h"
#include <algorithm>
#include <typeinfo>
#include <cstring>
...
#ifndef PERCIPIO_NO_SOO

#ifndef PERCIPIO_ENABLE_CPP11
	// C++11 needed for std::aligned_storage
	#error "Any SOO can only be enabled with C++11 support"
#endif

template <typename PlaceholderT, unsigned int SizeV = PERCIPIO_SMALL_OBJECT_SIZE>
union Placeholder
	/// ValueHolder union (used by Percipio::Any and Percipio::Dynamic::Var for small
	/// object optimization, when enabled).
	/// 
	/// If Holder<Type> fits into PERCIPIO_SMALL_OBJECT_SIZE bytes of storage, 
	...
	/// larger - [PERCIPIO_SMALL_OBJECT_SIZE + 1], additional byte value indicating
...
#if !defined(PERCIPIO_MSVC_VERSION) || (defined(PERCIPIO_MSVC_VERSION) && (PERCIPIO_MSVC_VERSION > 80))
private:
#endif
...
class Any
	...
	///
	/// Code taken from the Boost 1.33.1 library. Original copyright by Kevlin Henney. Modified for Percipio
...
public:
	...
	{
		char buf[PERCIPIO_SMALL_OBJECT_SIZE] = { 0 };
		return 0 == std::memcmp(_valueHolder.holder, buf, PERCIPIO_SMALL_OBJECT_SIZE);
	}
...
private:
	...
	public:
		...
		{
			if ((sizeof(Holder<ValueType>) <= PERCIPIO_SMALL_OBJECT_SIZE))
			{
