...
// VarHolder.h
//
// $Id: //percipio/svn/Foundation/include/Percipio/VarHolder.h#3 $
//
// Library: Foundation
// Package: Dynamic
// Module:  VarHolder
...
#include "Percipio/NumberParser.h"
#include "Percipio/DateTime.h"
#include "Percipio/Timestamp.h"
#include "Percipio/LocalDateTime.h"
#include "Percipio/DateTimeFormat.h"
#include "Percipio/DateTimeFormatter.h"
#include "Percipio/DateTimeParser.h"
#include "Percipio/String.h"
#include "Percipio/UnicodeConverter.h"
#include "Percipio/UTFString.h"
#include "Percipio/UTF8String.h"
#include "Percipio/Any.h"
...
class Foundation_API VarHolder
	...
	/// std::string, Percipio::UTF16String, DateTime, LocalDateTime, Timestamp, std::vector<Var> and DynamicStruct.
	///
	/// Additional types can be supported by adding specializations. When implementing specializations,
	/// the only condition is that they reside in Percipio namespace and implement the pure virtual functions
	/// clone() and type().
	///
...
public:
	...
	virtual VarHolder* clone(Placeholder<VarHolder>* pHolder = 0) const = 0;
		...
		/// PERCIPIO_NO_SOO is not defined), VarHolder will be
		/// instantiated in-place if it's size is smaller
		/// than PERCIPIO_SMALL_OBJECT_SIZE.
	...
	virtual void convert(Percipio::UTF16String& val) const;
	/// Throws BadCastException. Must be overriden in a type
	/// specialization in order to suport the conversion.
...
protected:
	...
	VarHolder* cloneHolder(Placeholder<VarHolder>* pVarHolder, const T& val) const
		...
		/// larger than PERCIPIO_SMALL_OBJECT_SIZE, holder is instantiated on
	...
	void convertToSmaller(const F& from, T& to) const
		...
		/// it is converted.
	{
		percipio_static_assert (std::numeric_limits<F>::is_specialized);
		percipio_static_assert (std::numeric_limits<T>::is_specialized);
		percipio_static_assert (std::numeric_limits<F>::is_signed);
		percipio_static_assert (std::numeric_limits<T>::is_signed);
		...
		/// If uper limit is within the target data type limits, the conversion is performed.
	{
		percipio_static_assert (std::numeric_limits<F>::is_specialized);
		percipio_static_assert (std::numeric_limits<T>::is_specialized);
		percipio_static_assert (!std::numeric_limits<F>::is_integer);
		percipio_static_assert (std::numeric_limits<T>::is_integer);
		percipio_static_assert (!std::numeric_limits<T>::is_signed);
...
inline void VarHolder::convert(Percipio::UTF16String& /*val*/) const
{
	throw BadCastException("Can not convert to Percipio::UTF16String");
}
...
public:
	...
	void convert(Percipio::UTF16String& val) const
	{
		std::string str = NumberFormatter::format(_val);
		Percipio::UnicodeConverter::convert(str, val);
	}
...
public:
	...
	void convert(Percipio::UTF16String& val) const
	{
		Percipio::UnicodeConverter::convert(_val, val);
	}
...
{
public:
	VarHolderImpl(const char* pVal) : _val(Percipio::UnicodeConverter::to<UTF16String>(pVal))
	{
	}

	VarHolderImpl(const Percipio::UTF16String& val) : _val(val)
	{
	}
	...
	const std::type_info& type() const
	{
		return typeid(Percipio::UTF16String);
	}
	...
	const Percipio::UTF16String& value() const
	{
		return _val;
	}
	...
	UTF16Char& operator[](Percipio::UTF16String::size_type n)
	{
		if (n < size()) return _val.operator[](n);
	...
	const UTF16Char& operator[](Percipio::UTF16String::size_type n) const
	{
		if (n < size()) return _val.operator[](n);
...
public:
	...
	void convert(std::string& val) const
	{
		val = DateTimeFormatter::format(_val, Percipio::DateTimeFormat::ISO8601_FORMAT);
	}
